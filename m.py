    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 600:
                ...
                response = f"Ooo CHOD NA KHATAM Finished. 🎯Target: {target}🏖️ Port: {port}⏰ Time: {time}"
                bot.reply_to(message, response)  # Notify the user  CHOD NE KA TIME KHATAM
        else:
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "ABE LAUDE CHOD NE KI UMAR NEHI HE TERE Use This Command 😡."
...
Official Channel :- https://t.me/V3NO
'''
